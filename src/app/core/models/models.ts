...
export type TableType = 'TABLE' | 'RESERVED';
...
export type FloorElementKind = 'wall' | 'zone' | 'door' | 'text';
export interface FloorPlanElement { id:string; floor_plan_id:string; kind:FloorElementKind; x:number; y:number; width:number; height:number; rotation:number; points:number[]|null; label:string|null; z_index:number; created_at:string; updated_at:string; }
...
export interface FloorSnapshot { elements:FloorPlanElement[]; tables:ClubTable[]; reserved?:ClubTable[]; }
