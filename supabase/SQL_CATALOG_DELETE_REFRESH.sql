-- Reaplica las RPC de borrado seguro del catálogo.
-- Ejecutar en Supabase SQL Editor si las funciones no existen o están desactualizadas.
...
-- Diagnóstico opcional: sustituye UUID_PRODUCTO para comprobar por qué un producto está bloqueado.
-- select oi.id, oi.order_id, oi.status, oi.quantity
-- from public.order_items oi
-- where oi.product_id = 'UUID_PRODUCTO'::uuid;
